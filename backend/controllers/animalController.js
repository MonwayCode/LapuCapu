...
const removeAnimal = (animalId, res) => {
  ...
  db.query(sql, [animalId], (err, results) => {
    ...
    if (results.affectedRows === 0) {
      return res
        .status(404)
        .json({ message: "Zwierzę nie zostało znalezione." });
    }
...
exports.updateAnimal = (req, res) => {
  ...
  upload(req, res, (err) => {
    ...
    const caretakerId = req.body.caretakerId
      ? parseInt(req.body.caretakerId, 10)
      : null;
    ...
    const values = imageURL
      ? [name, age, description, imageURL, categoryId, caretakerId, animalId]
      : [name, age, description, categoryId, caretakerId, animalId];
